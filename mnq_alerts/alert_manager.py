        ...
        for level in self._levels.values():
            if level.update(current_price):
                score = _composite_score(
                    level.name, level.entry_count, now_et,
                    tick_rate, session_move_pts,
                )

                if score < _MIN_SCORE:
                    print(f"[ALERT suppressed — low score ({score})] {level.name} zone entered "
                ...
                title, body = _build_message(
                    level.name, level.price, current_price,
                    level.entry_count, now_et,
                    score=score, tier_label=tier_label, tier_wr=tier_wr,
                )
...
def _time_bucket(t: datetime.time | None) -> str:
    ...
    if   mins < 11 * 60 + 30:  return "late morning"   # 10:30–11:30 (score -3 penalty)
    elif mins < 13 * 60:       return "lunch"           # 11:30–13:00, 70.9%
    elif mins < 15 * 60:       return "afternoon"       # 13:00–15:00, 77.6%
    else:                      return "power hour"      # 15:00–16:00, 75.7%
